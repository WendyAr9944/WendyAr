...
mainwindow::mainwindow(QWidget *parent)
    : QWidget(parent), currentWave(0), elapsedTime(0), isGameOver(false), showButtons(false), gameSpeed(1), selectedTower(nullptr)
{
    ...
    // 初始化障碍物
    obstacles.append(new Obstacle(QPoint(200, 200)));
    // 初始化炮塔坑
...
void mainwindow::mousePressEvent(QMouseEvent *event)
{
    ...
    for (TowerPit* pit : towerPits) {
            if (pit->getRect().contains(clickPos) && !pit->hasTower()) {
